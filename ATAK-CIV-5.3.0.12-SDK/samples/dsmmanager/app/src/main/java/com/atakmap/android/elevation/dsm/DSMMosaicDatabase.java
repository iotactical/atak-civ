...
public class DSMMosaicDatabase implements MosaicDatabase2 {
    ...
    public Cursor query(QueryParameters params) {
        ...
        Collection<Filter<MosaicDatabase2.Cursor>> filters = new ArrayList<>();
        if (params != null) {
    ...
    private static class CursorImpl implements Cursor {
        ...
        private void validateRow() {
            ...
            Set<DatasetDescriptor> descs = GdalLayerInfo.INSTANCE.create(
                    new DatasetDescriptorSpiArgs(this.list[this.idx], null));
        ...
        public GeoPoint getUpperLeft() {
            return this.row.upperLeft;
        }

        @Override
        public GeoPoint getUpperRight() {
            return this.row.upperRight;
        }

        @Override
        public GeoPoint getLowerRight() {
            return this.row.lowerRight;
        }

        @Override
        public GeoPoint getLowerLeft() {
            return this.row.lowerLeft;
        }
    ...
    private final static class ResolutionFilter
            implements Filter<MosaicDatabase2.Cursor> {
        ...
        public boolean accept(Cursor arg) {
            if (!Double.isNaN(this.minGsd) && (arg.getMaxGSD() > this.minGsd))
                return false;
            return Double.isNaN(this.maxGsd) || (!(arg.getMinGSD() < this.maxGsd));
        }
    }

    private final static class SpatialFilter
            implements Filter<MosaicDatabase2.Cursor> {
        private final Envelope filter;
    ...
    private final static class TypeFilter
            implements Filter<MosaicDatabase2.Cursor> {
    ...
    private final static class PathFilter
            implements Filter<MosaicDatabase2.Cursor> {
    ...
    private final static class SridFilter
            implements Filter<MosaicDatabase2.Cursor> {
